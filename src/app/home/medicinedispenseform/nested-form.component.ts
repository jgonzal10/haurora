...
<fieldset>
  <div *ngIf="itemsFormArray.hasError('minSum')">
    Las cantidades ingresadas para FLUORACILO deben sumar no menos que {{ itemsFormArray.getError('minSum') }}.
  </div>
  <item-control
...
<div class="form-group row" [formGroup]="item">
  <div class="col-sm-3">
    <label [attr.for]="'medicine'+index">Medicamento</label>
    <input type="text" class="form-control" [attr.id]="'medicine'+index" formControlName="medicine">
  </div>
    <div class="col-sm-3">
...
export class ItemFormControlComponent {
  ...
  static buildItem(val: string) {
    return new FormGroup({
      medicine: new FormControl(val, Validators.required),
